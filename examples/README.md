...
| `gbif_occurrence_sample.csv` | 4 | Tiny synthetic GBIF/Darwin Core-shaped file for testing `placebot-prep`. |

> `placebot-prep` / `placebot-expand` are not GBIF-specific — they work on any
> of the files above (native PlaceBot, Darwin Core, or GBIF). The GBIF samples
> are used below simply because they contain the repeated localities that make
...
To test the preparation command:
...
onto every original occurrence with `placebot-expand`:
