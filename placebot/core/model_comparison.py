...
class ModelComparison:
    ...
    def compare_models(
        ...
        for config in model_configs:
            ...
            comparison = {
                ...
                'estimated_time_minutes': ModelComparison._estimate_time(num_records, config),
    ...
    def display_comparison(
        ...
        print("\n🔍 MODEL COMPARISON")
        print("=" * 100)
        print(f"{'Rank':<6} {'Model Name':<30} {'Vendor':<15} {'Cost':<12} {'Time':<10} {'Caching':<10}")
        print("-" * 100)
        
        for i, comp in enumerate(comparisons, 1):
            cost_str = f"${comp['estimated_cost']:.4f}" if not comp['is_local'] else "FREE"
            time_str = f"{comp['estimated_time_minutes']:.1f} min"
            cache_str = "✅ Yes" if comp['has_caching'] else "❌ No"
            
            print(f"{i:<6} {comp['model_name']:<30} {comp['vendor']:<15} "
