...
### Changed
- **Ensemble output uses a fixed column layout.** Comparison files now write all
  of the primary file's columns (in their original order), then the secondary
  file's georeference columns prefixed with `Secondary_` (e.g.
  `Secondary_Latitude`, `Secondary_Confidence`, `Secondary_Processing_Notes`),
  and finally `Agreement_Category` followed by `Distance_km`. Previously only
  `Secondary_Latitude`/`Secondary_Longitude` were carried and columns were
  ordered generically.
